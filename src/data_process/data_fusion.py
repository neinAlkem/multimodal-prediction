...
if __name__ == "__main__":
    ...
    parser.add_argument('--participant-info', type=str, required=True, help='Path to participant info GCS directory.')
    parser.add_argument('--survey-info', type=str, required=True, help='Path to survey info GCS directory.')
    parser.add_argument('--class-table', type=str, required=True, help='Path to class table GCS directory.')
    parser.add_argument('--wearable-data', type=str, required=True, help='Path to wearable base GCS directory.')
    parser.add_argument('--output-path', type=str, required=True, help='GCS path for the final fused data output.')
    ...
    logging.info(f'Participant Info Dir: {args.participant_info}')
    logging.info(f'Survey Info Dir: {args.survey_info}')
    logging.info(f'Wearable Data Dir: {args.wearable_data}')
    logging.info(f'Fusion Output: {args.output_path}')
    
    try:
        df_survey = process_survey(spark, 
                                   args.survey_info_gcs,
                                   args.participant_info_gcs,
                                   args.class_table_gcs)

        df_wearable = process_wearable_data(spark,
                                            args.wearable_data_gcs)
        ...
        print(f"Saving final fused data as CSV to: {args.output_path}")
        df_join.to_csv(args.output_path, 
                             index=False, 
                             header=True, 
                             mode='w')
